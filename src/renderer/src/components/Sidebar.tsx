...
export default function Sidebar() {
  ...
  // Preference: starred (main) folder if set, else the default app folder.
  const modelsFolder = mainModelFolder || paths?.models || ''
  const backendFolder = mainBackendFolder || paths?.backend || ''

  // Friendly label: show the starred folder's basename, or "models"/"backend" for defaults.
  const modelsLabel = mainModelFolder ? mainModelFolder.split(/[/\\]/).pop() : 'models'
  const backendLabel = mainBackendFolder ? mainBackendFolder.split(/[/\\]/).pop() : 'backend'
  ...
  return (
    <nav className={`sidebar ${compactSidebarEnabled ? 'sidebar-compact' : ''}`}>
      ...
      {paths && (
        <div className="folder-triggers-container sidebar-folder-btn" style={{ marginTop: 'auto', paddingTop: 12 }}>
          <button
            className="nav-item"
            onClick={() => backendFolder && window.api.openFolder(backendFolder)}
            title={backendFolder}
          >
            <FolderOpen size={16} />
            <span className="nav-item-text">
              <span className="folder-label" style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                {mainBackendFolder && (
                  <span className="star-mark" style={{ color: '#f5b400' }}><StarShape active size={13} /></span>
                )}
                Open /{backendLabel}
              </span>
              {!mainBackendFolder && <span className="folder-sub">Default Backends Folder</span>}
              {mainBackendFolder && <span className="folder-sub">Main Backends Folder</span>}
            </span>
          </button>
          <button
            className="nav-item"
            onClick={() => modelsFolder && window.api.openFolder(modelsFolder)}
            title={modelsFolder}
          >
            <FolderOpen size={16} />
            <span className="nav-item-text">
              ...
              {!mainModelFolder && <span className="folder-sub">Default Models Folder</span>}
              {mainModelFolder && <span className="folder-sub">Main Models Folder</span>}
            </span>
          </button>
        </div>
      )}
