...
<!-- SCREENSHOT NEEDED: the Free VRAM box from Advanced Parameters, ideally
     ...
     and their apply buttons. -->

**Three-Way Configuration Presets**
Every template has a **Quick** / **FULL AUTO** / **Clear** switch:
- **Quick** applies a sane, fast-starting engine baseline (threads, batch sizes, flash attention, KV cache quantization, speculative decoding drafting parameters) and, for Dense models, the VRAM-recommended GPU layer count; MoE models are left on llama.cpp's own layer-splitting heuristic. Context defaults to the model's native context (capped at 32K for Dense; for MoE, estimated from your combined free VRAM+RAM headroom, since MoE tolerates RAM-resident layers well).
- **FULL AUTO** starts from the same baseline but hands context and GPU-layer placement over to llama.cpp's own `--fit` auto-sizing.
...
<!-- SCREENSHOT NEEDED: Monitoring tab, ideally with an active session
     showing both the generation-speed and prefill-speed charts populated. -->
